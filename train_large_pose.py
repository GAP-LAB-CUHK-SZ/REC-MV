...
resume_path = os.path.join(args.data, args.save_folder, 'a-pose.pth')
...
wv_resume = True if not debug else False
...
dataset,dataloader=getDatasetAndLoader(data_root,condlen,batch_size,
                        ...
                        curve_sampling = args.curve_sampling, a_pose = False)
# if debug:
...
# silluhute render 器
# rendernet work
optNet,sdf_initialized=getOptNet(dataset, args.save_folder, batch_size,bmins,bmaxs,resolutions['coarse'],device,config,use_initial_sdf ,visualizer= wv, opt_large = True)
...
if resume_path is not None and osp.isfile(resume_path):
    print('load model: '+resume_path,end='')
    if args.sdf_model is not None:
        print(' and substitute sdf model with: '+args.sdf_model,end='')
        sdf_initialized=-1
    optNet,dataset, start_epoch=utils.load_model(resume_path,optNet,dataset,device,args.sdf_model,args.model_rm_prefix)
    ...
    os.makedirs('./debug/cano_fl/', exist_ok = True)
    ...
    for data_index, (frame_ids, outs) in enumerate(dataloader):
        ...
        if data_index%1==0:
            for garment_idx in range(optNet.garment_size):
                ...
                outinfo='(%d/%d) (%s): loss = %.5f; color_loss: %.5f, eikonal_loss: %.5f'%(epoch,data_index,garment_name, loss.item(),optNet.info['{}_color_loss'.format(garment_name)],optNet.info['{}_grad_loss'.format(garment_name)])+ \
