...
if os.path.exists( os.path.join(args.rec_root, 'modified.ply')):
    trm_ply = trimesh.load(os.path.join(args.rec_root, 'modified.ply'))
    verts = torch.from_numpy(trm_ply.vertices).float().cuda()
    faces = torch.from_numpy(trm_ply.faces).long().cuda()
    TmpVs_list[1] = verts
    Tmpfs_list[1] = faces

garment_TmpVs, garment_Tmpfs = TmpVs_list[1:], Tmpfs_list[1:]

os.makedirs(osp.join(args.rec_root,'fl_meshs'),exist_ok=True)
...
for data_index, (frame_ids, outs) in enumerate(dataloader):
    ...
    # colors_list,imgs_list,def1imgs_list,defVs_list, smpl_list=optNet.infer_garment(garment_TmpVs,garment_Tmpfs, dataset.H, dataset.W,ratio,frame_ids,args.nColor,gts, args.rec_root)
    curve_mesh =optNet.infer_garment_fl(garment_TmpVs,garment_Tmpfs, dataset.H, dataset.W,ratio,frame_ids,args.nColor,gts, args.rec_root)


    fid= frame_ids[0].item()

    curve_mesh.export(osp.join(args.rec_root,'fl_meshs/{:06d}.obj'.format(fid)))
    print(osp.join(args.rec_root,'fl_meshs/{:06d}.obj'.format(fid)))
