'''
copy from https://github.com/papagina/RotationContinuity/blob/master/shapenet/code/tools.py
'''
import torch
...
def icp_rotate_center_transform(meshes, R,T):
    '''transform:
        a.icp translate [I], t
        a.center rotate [R], 0
    '''
    device = R.device
    # meshes, meshes_normal = self.feature_line_to_meshes(device)
    identity = repeat(torch.eye(3),'h w -> b h w', b = R.shape[0]).to(device)
    meshes_vertices = icp_rotate_transfrom(meshes, identity, T)

    R_T = torch.zeros_like(T).to(device)

    return center_transform(meshes_vertices, R, R_T)

def scale_icp_rotate_center_transform(meshes, R,T, scale):
    '''transform:
        a.icp translate [I], t
        a.center rotate [R], 0
    '''
    ...
    # meshes, meshes_normal = self.feature_line_to_meshes(device)
    identity = repeat(torch.eye(3),'h w -> b h w', b = R.shape[0]).to(device)
    meshes_vertices = scale_icp_rotate_transfrom(meshes, identity, T, scale)
    R_T = torch.zeros_like(T).to(device)
...
def center_transform(meshes, R, T):
    '''
    center rotated
    1. move feature line to center
    2. rotate feature line
    3. move back feature line to world-space
    '''

    device = R.device
    # meshes, meshes_normal = self.feature_line_to_meshes(device)
    if isinstance(meshes[0], pytorch3d.structures.Meshes):
        mesh_center = [mesh.verts_packed().mean(0, keepdim=True)[None] for mesh in meshes]
    else:
        mesh_center = [mesh.mean(0, keepdim=True)[None] for mesh in meshes]
    mesh_center = torch.cat(mesh_center, dim =0)
    identity = repeat(torch.eye(3),'h w -> b h w', b = R.shape[0]).to(device)

    # move to center point
    meshes_vertices = icp_rotate_transfrom(meshes, identity, -mesh_center)


    # rotate meshes
    meshes_vertices = icp_rotate_transfrom(meshes_vertices, R, T)
    # move back to origianl world-space
    meshes_vertices = icp_rotate_transfrom(meshes_vertices, identity, mesh_center)
    # compute normal change
    # with torch.no_grad():
    #     zero_T = torch.zeros(T.shape).to(device)
    #     meshes_normal = icp_rotate_transfrom(meshes_normal, R, zero_T)

    return meshes_vertices
def scale_icp_rotate_transfrom(meshes, R_pack, T_pack, S_pack):
    def transform(v, R, T, scale):
        ...
        scale = torch.clamp_min(scale,0.)
        v_scale = scale * init_scale
        v = center + v_scale * v_dirs
    ...
    transformed_vertices = [transform(vert, R, T, scale) for vert, R, T, scale in zip(vertices, R_pack, T_pack, S_pack)]
