...
dataset,dataloader = getDatasetAndLoader(osp.normpath(osp.join(args.rec_root,osp.pardir)),condlen,batch_size,
                        ...
                        config.get_bool('train.opt_pose'),config.get_bool('train.opt_trans'),config.get_config('train.opt_camera'), garment_type =config.get_string('train.garment_type'), data_type = args.data_type)
...
optNet, dataset, __=utils.load_model(osp.join(args.rec_root,'latest.pth'),optNet,dataset,device)
...
save_path = osp.join(args.rec_root,'animation/{}'.format(args.data_type))
os.makedirs(osp.join(save_path, 'colors'),exist_ok=True)
os.makedirs(osp.join(save_path, 'meshs'),exist_ok=True)
...
for data_index, (frame_ids, outs) in enumerate(dataloader):
    ...
    poses_y = outs['poses_y']

    frame_ids=frame_ids.long().to(device)
    colors_list, imgs_list, defVs_list=optNet.infer_garment_animation(garment_TmpVs,garment_Tmpfs, poses_y, dataset.H, dataset.W, ratio, frame_ids, save_path)

    for colors, imgs, defVs, garment_name in zip(colors_list, imgs_list, defVs_list, optNet.garment_names):
        for fid,img,defV in zip(frame_ids.cpu().numpy().reshape(-1),imgs,defVs):
            np.save(osp.join(save_path,'meshs/{}_{:06d}.npy'.format(garment_name, fid)),defV.reshape(-1,3))
            if not args.nI:
                cv2.imwrite(osp.join(save_path,'meshs/{}_{:06d}.png'.format(garment_name, fid)),img[:,:,[2,1,0]])
        if colors is not None:
            if not args.nI:
                for fid,color in zip(frame_ids.cpu().numpy().reshape(-1),colors):
                    cv2.imwrite(osp.join(save_path,'colors/{}_{:06d}.png'.format(garment_name, fid)),color)
